        ...
        try:
            ...
            for audio_chunk in audio_stream:
                ...
                if word_index < len(words) and audio_position > (word_index * 0.5):
                    await websocket.send_text(json.dumps({"type": "word_highlight", "word_index": word_index, "word": words[word_index], "timestamp": audio_position}))
                    word_index += 1
            ...
            logger.info("ElevenLabs TTS: Synthesis completed successfully")
        except Exception as e:
    ...
    async def _synthesize_fallback(self, text: str, websocket: WebSocket, tts_client_queue: asyncio.Queue):
        ...
        try:
            ...
            for audio_chunk in audio_chunks:
                ...
                await tts_client_queue.put(audio_chunk)
            await websocket.send_text(json.dumps({"type": "word_timing_complete", "total_words": len(words)}))
            logger.info("Fallback TTS: Synthesis completed successfully")
        except Exception as e:
        ...
        class TTSService:
            ...
            async def _synthesize_azure(self, text: str, websocket: WebSocket, tts_client_queue: asyncio.Queue):
                ...
                try:
                    ...
                    if result.reason == ResultReason.SynthesizingAudioCompleted:
                        ...
                        # Process audio chunks with word timing
                        audio_position = 0
                        word_index = 0
                        words = text.split()
                        ...
                        for audio_chunk in audio_chunks:
                            # Calculate approximate word timing
                            chunk_duration = len(audio_chunk) / 16000  # Azure uses 16kHz
            ...
            async def _synthesize_fallback(self, text: str, websocket: WebSocket, tts_client_queue: asyncio.Queue):
                ...
                try:
                    ...
                    # Generate a simple audio response (silence with beep)
                    sample_rate = 44100
                    duration = 2.0  # 2 seconds
                    samples = int(sample_rate * duration)
                    
                    # Create a simple beep tone (440 Hz sine wave)
                    import math
                    audio_data = bytearray()
                    for i in range(samples):
                    ...
                    for audio_chunk in audio_chunks:
                        # Calculate approximate word timing
                        chunk_duration = len(audio_chunk) / (sample_rate * 2)  # 16-bit samples
