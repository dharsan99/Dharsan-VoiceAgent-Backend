...
# GKE Production Deployment Script for Phase 4 (Smaller Resources)
# This script deploys the complete voice agent system to GKE production with smaller resource requirements

set -e

echo "🚀 Starting GKE Production Deployment for Phase 4 (Smaller Resources)..."
...
# Function to create GKE cluster with smaller resources
create_gke_cluster() {
    print_header "=== Creating GKE Cluster (Smaller Resources) ==="
    ...
    print_status "Creating GKE cluster: $CLUSTER_NAME (with smaller resources)"
    ...
    print_status "✅ GKE cluster created with smaller resources"
}

# Function to create AI workload node pool with smaller resources
create_ai_node_pool() {
    print_header "=== Creating AI Workload Node Pool (Smaller Resources) ==="
    
    # Check if node pool already exists
    if gcloud container node-pools describe "ai-workload-pool" --cluster="$CLUSTER_NAME" --region="$REGION" &> /dev/null; then
        print_warning "AI workload node pool already exists. Skipping creation."
        return
    fi
    
    print_status "Creating AI workload node pool (with smaller resources)"
    ...
    print_status "✅ AI workload node pool created with smaller resources"
}
...
update_manifests() {
    ...
    # Add node scheduling rules for AI services
    for file in "$TEMP_DIR"/stt-service-deployment.yaml "$TEMP_DIR"/tts-service-deployment.yaml "$TEMP_DIR"/llm-service-deployment.yaml; do
        if [ -f "$file" ]; then
            # Add nodeSelector and tolerations before the containers section
            sed -i.bak2 '/containers:/i\
          nodeSelector:\
            workload-type: ai\
          tolerations:\
            - key: "ai-workload"\
...
' "$file"
    ...
    print_status "✅ Manifests updated with production image paths"
...
# Main execution
main() {
    print_header "=== GKE Production Deployment Started (Smaller Resources) ==="
    ...
    print_header "=== GKE Production Deployment Complete (Smaller Resources) ==="
    ...
    echo "  - Smaller resource footprint for cost efficiency"
    echo ""
    print_status "🔧 Next Steps:"
